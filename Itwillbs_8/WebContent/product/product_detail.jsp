			...
			<div class="cboth width1260">
				...
				<div id="powerReview">
					...
					<div id="writePowerReview">
						<div class="PR15N01-write">
							<form name="prw_form" id="prw_form" method="post"
								...
								<!-- 별점 -->
								<div class="rat">
									<input type="number" name="starScore" id="rating1"
								...
								<input type="file" name="prw_file" class="trick file-attach"
									id="prw_file"> <input type="button" value="리뷰 등록"
									id="lnk-review" class="lnk-review"
					...
					<div class="PR15N01-hd" id="viewPoint">
						<h2>
							프리미엄 상품평<span>(<span class="totalReview_count"></span>)
							</span>
						</h2>
						<ul class="sort">
							<li class="now" value="date"><a
								href="javascript:getReplyCall();">최신 순</a></li>
							<li value="score"><a href="javascript:getReplyCall('','1');">평점
									순</a></li>
							<li value="good"><a href="javascript:getReplyCall();">추천
									순</a></li>
						</ul>
					</div>
					<div class="tabs">
						<ul class="reviewTabs">
							<li class="tab signin active"><a href="#signin">포토리뷰(<span
									class="photoReview_count"></span>)
							</a></li>
							<li class="tab signup"><a href="#signup">일반리뷰(<span
									class="review_count"></span>)
							</a></li>
						</ul>
					</div>
                  ...
                  %>
								...
								<td>
									<div class="tb-left reply_depth0">
										<span><a href="javascript:void(0)"
											onclick="show_hide(this); return false;"><%=qnaList.get(i).getQna_subject()%></a></span>
								...
								<td colspan="6">
									<div class="tb-left">
										<div class="qna_board_content">
											<div
												style="padding-bottom: 15px; padding-left: 80px; padding-right: 15px; padding-top: 15px">
												<%=qnaList.get(i).getQna_content() %>
											</div>
											<%if(member_id != null){if(qnaList.get(i).getMember_id().equals(member_id)){ %>
											<div class="pr_r_button">
												<a
													href="ProdQnaModifyForm.po?basicCode=<%=basicCode%>&qna_num=<%=qnaList.get(i).getQna_num()%>">수정</a>
												<a
													href="ProdQnaReplyForm.po?basicCode=<%=basicCode%>&qna_num=<%=qnaList.get(i).getQna_num()%>">답글</a>
												<a
													href="ProdQnaDeleteForm.po?basicCode=<%=basicCode%>&qna_num=<%=qnaList.get(i).getQna_num()%>">삭제</a>
											</div>
											<%} %>
											<%}else if(qnaList.get(i).getQna_re_seq()>0){ %>
											<div class="MS_cmt_list_box">
												<div class="comment_depth1">
													<table class="MS_cmt_list" border="0" cellspacing="0"
														cellpadding="0">
														<tbody>
															<tr>
					...
					<div class="list-btm">
						<div class="paging-wrap">
                           ...
                           %>
								<input type="button" value="이전"
									onclick="location.href='ProductDetail.po?basicCode=<%=basicCode%>&page=<%=nowPage - 1%>'">&nbsp;
								<%
                              }
                           %>
								<%
                              ...
                              %>
								...
								<%
                                 ...
                                 %>
								<a href="ProductDetail.po?basicCode=<%=basicCode%>" class="now">[<%=i%>]
								</a>&nbsp;
								<%
                                    }
                                 %>
				...
				<div class="deli_info_area">
					...
					<div class="deli_txt02">
						...
						글로벌 토탈 패션 브랜드 SHOP OOTD는 명품 하우스에서 사용하는 최고급 소재를 고집하여 세계 최고의 검사, 검증,<br>
						테스트를 인증하는 SGS*의 인증처에서 엄격한 품질, 공정관리를 거쳐 제작되었습니다. 패셔너블한 디자인으로 최상의 제품<br>
						퀄리티를 선보이는 동시에 최고의 가치소비를 지향하며 합리적인 가격대를 제시합니다.<br> SHOP OOTD의
						가죽은 천연가죽으로, 미세한 핏줄자국, 주름, 반점 등이 보일 수 있습니다. 천연가죽의 특성으로 인한 가죽의 상태와,<br>
						제거가 가능한 실밥, 봉제선의 미세한 흔들림 등 핸드메이드 공정 중 발생할 수 있는 현상들은 제품의 불량 사유에
						해당하지<br> 않으니, 참고 부탁 드립니다.<br> * SGS는 국제표준(ISO)절차를 준수하는
						세계 최고의 검사, 검증, 테스트 및 인증 회사입니다.
						<div class="deli_title02">
					...
					<div class="deli_txt01">
						...
						고객님께는 개런티 카드가 발송되며, 개런티 카드를 지참하셔야 A/S가 가능합니다.<br> 1) 전국 SHOP
						OOTD 매장 방문 접수 (면세점 접수 불가)<br> 2) 택배 접수<br> - 고객정보, 구매정보,
						수선 요청 사항 기재 / 개런티 카드 (또는 구매내역서) 동봉<br> - 택배지 주소 : (04995) 부산시
		   	                	...
		   	                	for (var j = 0; j < replyList[i].length; j++) {
		    	                	...
		    	                	if(replyList[i][0].id == 'admin'){
		    	                		if(j==0){
		    	                			...
		    	                			output +=		"<button class='pr_r_button'>";
